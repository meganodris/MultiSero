...
parameters {
 ...
 vector<lower=0,upper=1>[nPp] sero[nL]; // infection prevalence
...
transformed parameters {
  ...
  simplex[nC] theta[nL]; // gaussian weights
  vector[nP] seroAll = rep_vector(0,nP); // prevalence
  vector[nP] seroLoc[nL] = rep_array(rep_vector(0,nP),nL); // prevalence per loc
  ...
  // some temp variables  
  matrix[nC,nP] W[nL];
  vector[nC] log_theta[nL];
  int ix = 1;
  real cv;
  real sig;

  //--- infection prevalence ---//
  for(p in 1:nP) if(pres[p]==1) seroAll[p] = sum(to_vector(sero[,p]).*NperL)/N;
  for(l in 1:nL) for(p in 1:nP) if(pres[p]==1) seroLoc[l,p] = sero[l,p];


  //--- gaussian weights ---//
  for(c in 1:nC) for(p in 1:nP){
    if(infM[c,p]==0) for(l in 1:nL) W[l,c,p] = 1 - seroLoc[l,p];
    else for(l in 1:nL) W[l,c,p] = seroLoc[l,p];
  }
  for(c in 1:nC) for(l in 1:nL) theta[l,c] = prod(W[l,c,]);
  log_theta = log(theta);
  ...
  for(c in 1:nC) for(n in 1:N) pC[n,c] = log_theta[loc[n],c] + multi_normal_lpdf(y[n] | mu[c], covM[c]);
...
model {
 ...
 // priors
 for(p in 1:nPp) sero[,p] ~ beta(1,5);
 mu0 ~ normal(0,0.1);
 mu1 ~ normal(3,0.1);
 sd0 ~ normal(0.4,0.05);
 sd1 ~ normal(0.7,0.05);
